...
void funcion_productor(int id_propio) {
   const int mi_etiq = id_propio % 2 == 0 ? etiq_prod_par : etiq_prod_impar;
   for ( int i= 0 ; i < num_items_prod; i++ )
   {
      ...
      MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, mi_etiq, MPI_COMM_WORLD );
   }
}
// ---------------------------------------------------------------------
...
void funcion_consumidor(int id_propio)
{
   ...
   for( int i=0 ; i < num_items_cons; i++ )
   {
      ...
      MPI_Recv ( &valor_rec, 1, MPI_INT, id_buffer, etiq_respuesta_a_cons, MPI_COMM_WORLD,&estado );
...
void funcion_buffer()
{
   ...
   int tope = num_items*2, etiq_aceptable;
   bool modo_1 = true;
   int id_aceptable;
   ...
   for( int i=0 ; i < tope ; i++ )  {
      ...
      if ( num_celdas_ocupadas == 0 ) {                   // solo prods.
        if (modo_1)
          etiq_aceptable = etiq_prod_impar ;
        else
          etiq_aceptable = etiq_prod_par ;
      } else if ( num_celdas_ocupadas == tam_vector ) {   // solo cons.
        etiq_aceptable = etiq_cons;
      } else {                                            // cualquiera
         MPI_Probe( MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &estado );
         if (estado.MPI_TAG == etiq_cons || (modo_1 ^ (estado.MPI_TAG != etiq_prod_impar) ) ) {
           id_aceptable = estado.MPI_SOURCE;
           etiq_aceptable = MPI_ANY_TAG;
         } else {
           etiq_aceptable = modo_1 ? etiq_prod_impar : etiq_prod_par;
         }
      }

      // 2. recibir un mensaje del emisor o emisores aceptables

      MPI_Recv( &valor, 1, MPI_INT, id_aceptable, etiq_aceptable, MPI_COMM_WORLD, &estado );

      // 3. procesar el mensaje recibido

      switch ( estado.MPI_TAG ) {// leer emisor del mensaje en metadatos
        case etiq_prod_par:
        case etiq_prod_impar:
          buffer[primera_libre] = valor ;
          primera_libre = (primera_libre+1) % tam_vector ;
          num_celdas_ocupadas++ ;
          cout << "\t\tBuffer ha recibido valor " << valor << ". Nuevo modo: " << modo_1+1 << endl ;
          modo_1 = !modo_1;
          break;
        ...
        case etiq_cons:
          ...
          MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE, etiq_respuesta_a_cons, MPI_COMM_WORLD);
          break;
      }
   }
}
