...
void funcion_hebra( MBarreraParSC * mon_barrera1, int num_hebra )
{
   while( true )
   {  const int ms = aleatorio<5,100>();
      this_thread::sleep_for( chrono::milliseconds(ms) );
      mon_barrera1->cita( num_hebra );
   }
}
// *****************************************************************************
...
int main()
{
   ...
   // crear monitor
   auto * mon_barrera2 = new MBarreraParSC( num_hebras_cita );

   // crear y lanzar hebras
   thread hebra[num_hebras];
   for( unsigned i = 0 ; i < num_hebras ; i++ )
      hebra[i] = thread( funcion_hebra, mon_barrera2, i );
